...
2. **Identify token type** based on first character:
   ...
   - Digit → Constant (with error checking for invalid formats)
   - Quote → String literal
   - Operator character → Operator (check for 2-char operators)
...
├── test1.c               # Test case 1 (with error)
├── test2.c               # Test case 2 (salary calculation)
└── test3.c               # Test case 3 (struct example)
```
...
A straightforward C++ lexical analyzer is added as `3.cpp`.
...
### Notes
- Reads a C source file and prints token categories in a simple format.
- Prints a basic symbol table at the end.
- Existing C files (`3.c`, `3test1.c`, `3test2.c`, `3test3.c`) are unchanged.
